...
import { SearchLogo } from '@/components/chat/chatroom/search/SearchLogo';
...
const ChannelSearchList = ({
        ...
        if (message.messageType != 'S') {
          if (message.messageID == moveId) {
            ...
            if (message.messageType === 'I') {
              let isMineFlag = false;

              if (message.isMine === 'Y') {
                isMineFlag = true;
              } else isMineFlag = false;

              if (message.isMine == null) isMineFlag = true;
              returnJSX.push(
  ...
  const handleScrollBottom = useCallback(() => {
    // console.log(beforeMessages.length, bottomEnd);
