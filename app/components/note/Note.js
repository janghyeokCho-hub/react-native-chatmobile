...
import { Text, View, StyleSheet, FlatList, TouchableOpacity, Keyboard, Platform, Alert, AppState } from 'react-native';
import {
...
function NoteManager({ viewType, isLoading, onSearch }) {
    const { data: searchText, mutate: setSearchText } = useSWR('/note/list/searchText', null, { initialData: '' });
...
function ViewTypeSelector({ onViewTypeChanged }) {
    ...
    const DrawButtons = useMemo(() => {
        return buttons.map((button, idx) => {
            ...
            return (
                <TouchableOpacity style={buttonStyle} onPress={() => setViewType(button?.viewType)} key={idx}>
                    <Text style={textColor}><button.Icon {...textColor} /> {button.text}</Text>
                </TouchableOpacity>
            );
        });
    }, [viewType]);
...
function SortTypeSelector({ viewType }) {
    ...
    const sortNoteList = useCallback(async (_sortName, _sort, setState) => {
    ...
    const getActiveStyle = useCallback((sortName) => {
                    ...
                    async onPress() {
                        try {
                            ...
                            if (data && data.status === 'SUCCESS') {
                                ...
                                Alert.alert(
                                    getDic('Note'),
                                    getDic('Msg_Note_DeleteAllSuccess')
                                );
                                setNameSort(SORT.DESC);
                                setDateSort(SORT.DESC);
                                setActiveSort(null);
    ...
    return (
        <View style={_styles.sortContainer}>
            <TouchableOpacity style={_styles.sortColumn} onPress={() => sortNoteList(SORT.DATE, dateSort, setDateSort)}>
                <Text style={getActiveStyle(SORT.DATE)}>
                    {getDic('Date')}
                </Text>
                <DirectionIcon direction={dateSort === SORT.DESC ? 'down' : 'up'} width={10} height={10} style={{ marginLeft: 4 }} />
            </TouchableOpacity>
            <TouchableOpacity style={_styles.sortColumn} onPress={() => sortNoteList(SORT.NAME, nameSort, setNameSort)}>
                <Text style={getActiveStyle(SORT.NAME)}>
                    {getDic('Name')}
                </Text>
                <DirectionIcon direction={nameSort === SORT.DESC ? 'down' : 'up'} width={10} height={10} style={{ marginLeft: 4 }} />
            </TouchableOpacity>
...
export default function Note({ navigation }) {
    ...
    const { data: noteList, initNoteList, isValidating, error, search } = useNoteList({ viewType });
        ...
        <View style={styles.container}>
            ...
            <View style={styles.contentWrap}>
                <View style={styles.contents}>
                    <NoteManager viewType={viewType} isLoading={isLoading} onSearch={search} />
                    <ViewTypeSelector />
                    <SortTypeSelector viewType={viewType} />
                    <NoteList data={noteList} error={error} />
                    {Platform.OS === 'android' && noteList && isLoading && <Circle indeterminate={true} style={{ position: "absolute", bottom: 0, right: 0, endAngle: 0.6 }} color={colors.primary} />}
                </View>
            </View>
        </View>
    );
