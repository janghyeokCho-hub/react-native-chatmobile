...
function TabSelector({ style, tab, onChange }) {
  return (
    <View style={style}>
      ...
      <TouchableOpacity
        onPress={() => onChange?.('C')}
        style={[styles.tabItem, tab === 'C' && styles.tabItemActive]}
      >
      ...
      <TouchableOpacity
        onPress={() => onChange?.('O')}
        style={[styles.tabItem, tab === 'O' && styles.tabItemActive]}
      >
...
const AddTarget = ({ navigation, route }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header
        title={headerName}
        addCount={newTargetList.length}
        onFinish={handleFinish}
      />
      <Selected
        data={oldTargetList}
        onChange={setNewTargetList}
...
export default withSecurityScreen(AddTarget);
