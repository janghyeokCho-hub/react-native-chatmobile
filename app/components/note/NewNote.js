...
function _smallProfileBox({ name, photoPath }) {
    ...
    if (imgVisible) {
        return (
            <Image style={[styles.profileBox, styles.profileImage]} source={{ uri: photoPath || '.' }} onError={e => setImgVisible(false)} />
        );
    } else {
        return (
            <View style={[styles.profileBox, styles.profileTextContainer, { backgroundColor: nameColor }]} >
...
function RecipientItem({ user, onDelete }) {
    ...
    const optionSelected = useCallback((value) => {
        switch (value) {
            case 'Detail':
                user?.id && navigation.navigate('ProfilePopup', {
                    targetID: user.id,
                });
                break;
            case 'Delete':
    ...
    const profileBox = <SmallProfileBox name={userName} photoPath={user?.photoPath} />;
    ...
    return (
        <Menu onSelect={optionSelected}>
            <MenuTrigger style={styles.profileBoxContainer}>
                {/* { profileBox } */}
                <Text numberOfLines={1} style={{ fontSize: sizes?.default, paddingRight: '1%' }}>{userName}</Text>
            </MenuTrigger>
            <MenuOptions>
                {
...
function RecipientList({ ...rest }) {
    ...
    const handleAddTarget = useCallback((prev, added) => {
    ...
    const renderItem = useCallback(({ item }) => <RecipientItem user={item} onDelete={onDelete} />, [targetList]);
    const keyExtractor = useCallback(item => `${item?.id}_${item?.jobKey}_${item?.type}`, [targetList]);
    ...
    const collapseCount = useMemo(() => {
        ...
        return Math.max(
            targetList?.length - collapseThreshold,
            0
        );
    }, [targetList, collapsed]);

    const collapsedFooter = (
        <TouchableOpacity style={[styles.profileBoxContainer, { alignSelf: "baseline" }]} onPress={() => setCollapsed(prev => !prev)}>
            <Text style={{ color: '#3f51b5' }}>{'+'}{collapseCount}</Text>
        </TouchableOpacity>
    );
        ...
        <View {...rest}>
            ...
            <View style={{ flex: 4, flexDirection: "row" }}>
                ...
                <FlatList
                    ...
                    columnWrapperStyle={{ alignItems: "center", flexDirection: "row", flexWrap: 'wrap' }}
                />
            </View>
            <View style={{ flex: 1, flexDirection: "row", alignItems: "center" }}>
                <TouchableOpacity style={{ marginHorizontal: 8 }} onPress={handleAddTarget}>
                    <AddChannelIcon width={20} height={20} style={{ borderRadius: 10, borderWidth: 1, borderColor: '#ababab' }} color="#666" />
                </TouchableOpacity>
                <TouchableOpacity style={{ marginHorizontal: 2, padding: 6 }} onPress={() => setCollapsed(prev => !prev)}>
                    <DirectionIcon width={12} height={12} direction={collapsed ? 'down' : 'up'} />
                </TouchableOpacity>
            </View>
        </View>
    );
...
function NewNote({ navigation, route }) {
    ...
    const note = useMemo(() => {
        ...
        const { sender, receivers } = parseSender(noteInfo, { useOrgChartFormat: true });
    ...
    const { data: targetList, mutate: setTargetList } = useSWR('/note/send/target', null, { initialData: note.targets });
    ...
    async function handleSend() {
        ...
        try {
            console.log('Send Note   ', sendData);
